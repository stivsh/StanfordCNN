"""
Perform tests on LogisticClassifier classifier
"""
import unittest
...
from commonftests import numerical_dj, compare_analitic_and_numerical_dj,\
    simple_optimizer
from mylogicregression import j, h, dj, LogisticClassifier, OneVersusAllLogisticClassifier
...
class TestLogisticClassifier(unittest.TestCase):
    ...
    def test_2class_lr(self):
        ...
        clf = LogisticClassifier(alpha=0.001, optimizer=simple_optimizer).fit(
    ...
    def test_one_versus_all_lr(self):
        scaler = RobustScaler()
        train_labels = self.data['train_labels']
        train_dataset = self.data['train_dataset']
        test_labels = self.data['train_labels']
        ...
        clf = OneVersusAllLogisticClassifier(alpha=0.001, optimizer=simple_optimizer).fit(
            train_dataset, train_labels)
